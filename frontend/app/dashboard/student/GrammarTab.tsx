...
export default function GrammarTab({ API_URL }: GrammarTabProps) {
  ...
  if (practicing) {
    return (
      <div className="space-y-4 min-h-[80vh] flex flex-col items-center justify-center">
        {questions.length === 0 ? (
          <div className="app-card p-10 flex flex-col items-center max-w-md w-full text-center">
            <div className="relative mb-6">
              <div className="w-16 h-16 border-4 border-[var(--brand-soft)] border-t-[var(--brand)] rounded-full animate-spin" />
              <Sparkles className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[var(--brand)]" size={24} />
            </div>
            <h3 className="text-xl font-bold text-gray-900 mb-2">Đang tải bài tập...</h3>
            <p className="text-gray-400 font-medium text-sm">Vui lòng đợi!</p>
          </div>
        ) : (
          <div className="fixed inset-0 !mt-0 z-[150] flex flex-col bg-white overflow-y-auto">
            ...
            <div className="sticky top-0 bg-white/90 backdrop-blur-xl z-10 px-4 sm:px-6 py-3 flex items-center gap-3 border-b border-gray-100">
              ...
              <div className="flex-1 h-3 bg-gray-100 rounded-full overflow-hidden">
                <div className="h-full bg-[var(--accent)] transition-all duration-700 rounded-full"
                  style={{ width: `${(currentIdx / questions.length) * 100}%` }} />
              </div>
              <div className="flex items-center gap-2">
                <span className={`hidden sm:flex items-center gap-1 px-2 py-1 rounded-lg font-semibold text-xs border ${practiceMode === "choice" ? "bg-blue-50 text-blue-700 border-blue-100" : "bg-orange-50 text-orange-700 border-orange-100"}`}>
                  {practiceMode === "choice" ? <ListChecks size={11} /> : <Keyboard size={11} />}
                  {practiceMode === "choice" ? "Choice" : "Typing"}
                </span>
                <span className="font-semibold text-[var(--brand)] text-base bg-[var(--brand-soft)] px-3 py-1.5 rounded-xl border border-[var(--brand)]/20 whitespace-nowrap">{currentIdx + 1}/{questions.length}</span>
              </div>
            </div>
            ...
            {!submitted ? (() => {
              ...
              return (
                <div className="flex-1 max-w-4xl w-full mx-auto p-4 md:p-10 flex flex-col pt-8">
                  <div className="mb-8 text-center">
                    <span className="inline-flex items-center gap-1.5 px-4 py-1.5 bg-[var(--brand-soft)] text-[var(--brand)] rounded-xl font-semibold uppercase tracking-widest text-xs mb-5 border border-[var(--brand)]/20">
                      <GraduationCap size={13} />
                      {practiceMode === "typing" ? "Nhập đáp án" : q.type === "FIB" ? "Điền vào chỗ trống" : q.type === "TFNG" ? "Đúng / Sai / Không có" : "Chọn đáp án đúng"}
                    </span>
                    <h2 className="text-2xl md:text-3xl font-bold text-gray-900 leading-snug max-w-2xl mx-auto">{q.question}</h2>
                  </div>
                    ...
                    {showChoiceMode ? (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {q.options.map((opt: string, idx: number) => {
                          ...
                          let cls = "bg-white border-gray-100 hover:border-[var(--brand)] hover:bg-[var(--brand-soft)]/30 text-gray-700 shadow-sm";
                          ...
                          } else if (isSelected) cls = "bg-[var(--brand-soft)] border-[var(--brand)] text-slate-800 shadow-md scale-[1.01] ring-2 ring-[var(--brand)]/20";
                          return (
                            <button key={idx} disabled={questionSubmitted}
                              ...
                              <div className={`w-7 h-7 rounded-full border-2 flex items-center justify-center font-semibold flex-shrink-0 text-xs transition-all ${questionSubmitted && isCorrect ? "border-green-600 text-green-700 bg-white" : questionSubmitted && isSelected ? "border-red-500 text-red-600 bg-white" : isSelected ? "border-[var(--brand)] text-[var(--brand)] bg-white" : "border-gray-200 text-gray-400"}`}>
                      ...
                      <div className="relative max-w-lg mx-auto">
                        ...
                        <input autoFocus type="text"
                          ...
                          className={`w-full text-2xl font-bold text-center p-6 border-b-4 rounded-2xl outline-none transition-all shadow-lg ${questionSubmitted ? (isAnswerCorrect(answers[currentIdx] || "", q.answer) ? "border-green-500 bg-green-50 text-green-700" : "border-red-500 bg-red-50 text-red-700") : "border-gray-200 bg-gray-50 focus:border-[var(--brand)] focus:bg-white"}`}
                      ...
                      <div className="flex-1 w-full">
                        {questionSubmitted && (
                          <div className="flex items-center gap-3 animate-in slide-in-from-left-4">
                            ...
                            <div>
                              <h3 className={`font-semibold text-base ${isAnswerCorrect(String(answers[currentIdx]||""), String(q.answer||"")) ? "text-green-800" : "text-red-800"}`}>
                                {isAnswerCorrect(String(answers[currentIdx]||""), String(q.answer||"")) ? "Xuất sắc! 🔥" : "Sai rồi!"}
                              </h3>
                              <p className={`font-medium text-sm mt-0.5 ${isAnswerCorrect(String(answers[currentIdx]||""), String(q.answer||"")) ? "text-green-700" : "text-red-700"}`}>
                                Đáp án: <span className="px-2 py-0.5 bg-white rounded-lg ml-1 text-xs font-semibold shadow-sm">{q.answer}</span>
                              </p>
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-6 bg-gray-50/50">
                <div className="app-card p-10 md:p-12 text-center max-w-sm w-full">
                  <Trophy size={80} className="mx-auto text-yellow-400 drop-shadow-lg mb-6 animate-bounce" />
                  <h3 className="text-2xl font-bold mb-2 text-gray-900">Hoàn thành!</h3>
                  <p className="text-gray-400 font-medium mb-6 text-sm uppercase tracking-widest">Bạn đã vượt qua thử thách</p>
                  <div className="bg-[var(--brand-soft)] rounded-[var(--r-xl)] py-6 my-6 border border-[var(--brand)]/20">
                    <p className="text-[var(--brand)] font-semibold uppercase tracking-widest text-xs mb-1">Điểm số</p>
                    <p className="text-4xl font-bold tabular-nums text-[var(--brand)]">{score}<span className="text-xl text-[var(--brand)]/40">%</span></p>
                    <p className="text-[var(--brand)]/60 font-medium text-xs mt-1">{questions.filter((_,i) => isAnswerCorrect(answers[i]||"",questions[i]?.answer||"")).length}/{questions.length} câu đúng</p>
                  </div>
                  <button onClick={() => setPracticing(false)}
                    className="w-full bg-[var(--brand)] hover:bg-[var(--brand-dark)] text-white px-6 py-4 rounded-[var(--r-lg)] font-semibold transition-all shadow-lg text-base active:scale-95">
  ...
  if (selectedRule) {
    ...
    return (
      <div className="space-y-5 max-w-4xl mx-auto pb-16 animate-in fade-in duration-300">
        <div className="app-card flex flex-col md:flex-row justify-between items-start md:items-center gap-4 p-5">
          <div className="flex items-center gap-4">
            <div className="bg-[var(--brand-soft)] p-3 rounded-2xl"><BookOpen className="text-[var(--brand)]" size={28} /></div>
            <div>
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                <span className={`text-xs font-semibold px-2.5 py-0.5 rounded-full border ${lvlCfg.chip}`}>{lvlCfg.label} · {lvlCfg.sublabel}</span>
                {selectedRule.quiz_count > 0 && <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-[var(--brand-soft)] text-[var(--brand)] border border-[var(--brand)]/20">{selectedRule.quiz_count} bài tập</span>}
              </div>
              <h2 className="text-xl md:text-2xl font-bold text-gray-900">{selectedRule.name}</h2>
              <p className="text-xs font-medium text-gray-400 mt-0.5">{new Date(selectedRule.created_at).toLocaleDateString("vi-VN")}</p>
            </div>
          </div>
          <button onClick={() => setSelectedRule(null)}
            className="group bg-gray-50 hover:bg-gray-900 text-gray-500 hover:text-white px-5 py-3 rounded-xl font-semibold flex items-center gap-2 transition-all active:scale-95 shadow-sm text-sm">
            <ChevronLeft size={16} className="group-hover:-translate-x-1 transition-transform" /> Quay lại
          </button>
        </div>

        <div className="bg-white p-6 md:p-10 rounded-[var(--r-2xl)] border border-gray-100 shadow-lg">
        ...
        {selectedRule.quiz_count > 0 && (
          <div className="bg-[var(--brand-soft)] p-5 rounded-[var(--r-xl)] border border-[var(--brand)]/20">
            <h3 className="font-semibold text-gray-900 text-base mb-1 flex items-center gap-2">
              <ListChecks size={18} className="text-[var(--brand)]" /> Bài tập có sẵn ({selectedRule.quiz_count} câu)
            </h3>
            <p className="text-gray-500 font-medium text-xs mb-4">Luyện tập không cần AI credits — điểm được ghi nhận.</p>
            <div className="flex flex-wrap gap-2">
              <button onClick={() => { setPendingStoredRuleId(selectedRule.id); setShowPracticeConfig(true); setSelectedRule(null); }}
                className="bg-[var(--brand)] text-white px-5 py-2.5 rounded-xl font-semibold flex items-center gap-1.5 shadow hover:bg-[var(--brand-dark)] transition-all active:scale-95 text-sm">
              ...
              {isAdmin && (
                ...
                }} className="bg-red-50 text-red-600 border border-red-200 px-4 py-2.5 rounded-xl font-semibold flex items-center gap-1.5 hover:bg-red-100 transition-all text-sm">
                  <Trash2 size={13} /> Xóa
                </button>
              )}
            </div>
        ...
        <button onClick={() => { toggleRule(selectedRule.id); setSelectedRule(null); }}
          className={`w-full py-4 rounded-2xl font-semibold text-base flex items-center justify-center gap-2 transition-all active:scale-95 shadow-md ${selectedRules.includes(selectedRule.id) ? "bg-red-50 text-red-600 border-2 border-red-200 hover:bg-red-100" : "bg-[var(--brand)] text-white hover:bg-[var(--brand-dark)]"}`}>
  ...
  const RuleCard = ({ rule, depth = 0 }: { rule: any; depth?: number }) => {
    ...
    return (
      <div className={depth > 0 ? `ml-3 sm:ml-5 border-l-2 border-gray-100 pl-2 sm:pl-3 mt-2` : ""}>
        <div className={`group relative overflow-hidden rounded-2xl transition-all duration-300 cursor-pointer ${isSelected ? "ring-2 ring-[var(--brand)]/50 shadow-lg" : "hover:shadow-sm"}`}
          style={{ background: depth === 0 ? undefined : undefined }}>
          <div className={`flex items-start gap-2 p-3 sm:p-4 rounded-2xl border transition-colors ${isSelected ? "bg-[var(--brand-soft)] border-[var(--brand)]/20" : "bg-white border-gray-100 hover:border-[var(--brand)]/20 hover:bg-gray-50/50"}`}>
            {/* Expand toggle */}
            {hasChildren ? (
              <button onClick={e => { e.stopPropagation(); toggleExpand(rule.id); }}
                className="mt-0.5 text-gray-400 hover:text-[var(--brand)] transition-colors flex-shrink-0">
            ...
            <div className="flex-1 min-w-0" onClick={() => setSelectedRule(rule)}>
              ...
              <h4 className={`font-semibold text-sm tracking-tight leading-tight ${isSelected ? "text-[var(--brand)]" : "text-gray-900 group-hover:text-[var(--brand)]"}`}>{rule.name}</h4>
            ...
            <div className="flex items-center gap-1 flex-shrink-0">
              {quizCount > 0 && (
                <button onClick={e => { e.stopPropagation(); setPendingStoredRuleId(rule.id); setShowPracticeConfig(true); }}
                  className="text-[9px] font-semibold px-1.5 py-1 bg-[var(--brand)] text-white rounded-lg hover:bg-[var(--brand-dark)] transition-all flex items-center gap-0.5 whitespace-nowrap">
                  <ListChecks size={9} /> Luyện
                </button>
              )}
              <div className={`w-5 h-5 rounded-lg border-2 flex items-center justify-center transition-all cursor-pointer ${isSelected ? "bg-[var(--brand)] border-[var(--brand)] text-white" : "border-gray-200 bg-white hover:border-[var(--brand)]"}`}
  ...
  return (
    <div className="animate-in fade-in duration-300 pb-28">
      ...
      {/* ── Hero ── */}
      <div className="bg-[var(--brand)] rounded-[var(--r-xl)] sm:rounded-[var(--r-2xl)] p-4 sm:p-6 md:p-8 mb-5 text-white relative overflow-hidden shadow-lg shadow-blue-200/40">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-semibold text-blue-100 uppercase tracking-widest text-xs">Kho Ngữ Pháp</span>
            </div>
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold tracking-tight leading-none mb-1">Grammar Exercises</h1>
            <p className="text-blue-100 font-medium text-xs sm:text-sm">Luyện tập ngữ pháp theo trình độ CEFR — điểm được ghi nhận</p>
            <div className="flex flex-wrap gap-2 mt-3">
              ...
              <div className="flex items-center gap-1.5 bg-white/10 rounded-xl px-3 py-1.5 border border-white/10 text-xs">
                ...
                <span className="text-blue-100 font-medium">AI Practice</span>
              </div>
            </div>
          </div>
          <div className="flex flex-row sm:flex-row gap-2 flex-shrink-0">
            <button onClick={() => { setShowParseModal(true); setParseTab(isAdmin ? "local" : "ai"); setLocalParsedQuestions([]); setSavedSuccess(false); }}
              className="bg-white/15 hover:bg-white/25 border border-white/30 text-white px-4 py-2.5 rounded-xl font-semibold text-sm flex items-center gap-1.5 transition-all active:scale-95">
            ...
            <button onClick={() => { if (selectedRules.length > 0) setShowPracticeConfig(true); else showAlert("Chọn chủ đề từ danh sách bên dưới!", "warning"); }}
              className="bg-white text-[var(--brand)] hover:bg-blue-50 px-4 py-2.5 rounded-xl font-semibold text-sm flex items-center gap-1.5 shadow-lg transition-all active:scale-95">
              <Sparkles size={16} />
      ...
      {/* ── Sticky Practice Bar ── */}
      {selectedRules.length > 0 && (
        <div className="fixed bottom-0 left-0 lg:left-[220px] right-0 z-50 p-3">
          <div className="max-w-2xl mx-auto bg-white rounded-2xl border border-gray-100 shadow-xl p-3 flex items-center gap-3">
            <div className="flex-1">
              <p className="font-semibold text-[var(--ink-1)] text-sm">{selectedRules.length} chủ đề đã chọn</p>
              <p className="text-gray-400 text-xs font-medium truncate">{selectedRules.map(id => rules.find(r=>r.id===id)?.name).filter(Boolean).join(" · ")}</p>
            </div>
            <button onClick={() => setShowPracticeConfig(true)}
              className="bg-[var(--brand)] hover:bg-[var(--brand-dark)] text-white px-5 py-2 rounded-xl font-semibold flex items-center gap-1.5 shadow-lg transition-all active:scale-95 text-sm whitespace-nowrap">
      ...
      {showPracticeConfig && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative z-10 bg-white w-full max-w-sm rounded-2xl shadow-2xl p-6">
            ...
            <div className="mb-4">
              <p className="font-semibold text-gray-700 text-xs uppercase tracking-widest mb-2">Chế độ làm bài</p>
              <div className="grid grid-cols-2 gap-2">
                {[{ val: "choice" as PracticeMode, icon: <ListChecks size={18} />, label: "Choice", sub: "Chọn A/B/C/D", color: "teal" },
                  ...
                  <button key={m.val} onClick={() => setPracticeMode(m.val)}
                    ...
                    <p className={`font-semibold text-xs mt-1 ${practiceMode === m.val ? `text-${m.color}-700` : "text-gray-700"}`}>{m.label}</p>
            ...
            {!pendingStoredRuleId && (
              <div className="mb-5">
                <p className="font-semibold text-gray-700 text-xs uppercase tracking-widest mb-2">Độ khó (AI)</p>
                <div className="grid grid-cols-3 gap-2">
                  {[{ val: "Easy", label: "Cơ bản", sub: "A1-A2", ac: "border-green-500 bg-green-50 text-green-700" },
                    ...
                    <button key={d.val} onClick={() => setDifficulty(d.val)}
                      ...
                      <p className={`font-semibold text-xs ${difficulty === d.val ? "" : "text-gray-700"}`}>{d.label}</p>
            ...
            <div className="flex gap-2">
              <button onClick={() => { setShowPracticeConfig(false); setPendingStoredRuleId(null); }}
                className="flex-1 py-3 rounded-xl font-semibold text-sm text-gray-500 bg-gray-100 hover:bg-gray-200 transition">Hủy</button>
              {pendingStoredRuleId ? (
                <button onClick={() => startStoredQuizPractice(pendingStoredRuleId)}
                  className="flex-1 py-3 rounded-xl font-semibold text-sm text-white bg-[var(--brand)] hover:bg-[var(--brand-dark)] transition shadow-md flex items-center justify-center gap-1.5">
          ...
          <div className="relative z-10 bg-white w-full sm:max-w-2xl rounded-t-[28px] sm:rounded-2xl shadow-2xl flex flex-col max-h-[92vh]">
            ...
            <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-gray-100 flex-shrink-0">
              <div className="flex items-center gap-2.5">
                <div className="bg-[var(--brand-soft)] p-2 rounded-xl"><ClipboardPaste size={18} className="text-[var(--brand)]" /></div>
                <div>
                  <h3 className="text-base font-semibold text-gray-900">Phân tích đề thi</h3>
            ...
            <div className="flex border-b border-gray-100 px-5 flex-shrink-0">
              <button onClick={() => setParseTab("ai")}
                className={`pb-2.5 pt-3 px-3 font-semibold text-xs flex items-center gap-1.5 border-b-2 transition-all ${parseTab === "ai" ? "border-[var(--brand)] text-[var(--brand)]" : "border-transparent text-gray-400 hover:text-gray-700"}`}>
                <Sparkles size={13} /> AI Phân tích <span className="bg-[var(--brand-soft)] text-[var(--brand)] px-1.5 py-0.5 rounded-full text-[9px]">3 credits</span>
              </button>
              <button onClick={() => setParseTab("local")}
                className={`pb-2.5 pt-3 px-3 font-semibold text-xs flex items-center gap-1.5 border-b-2 transition-all ${parseTab === "local" ? "border-orange-500 text-orange-700" : "border-transparent text-gray-400 hover:text-gray-700"}`}>
            ...
            <div className="p-5 flex-1 overflow-y-auto space-y-3">
              {parseTab === "ai" ? (
                <>
                  <div className="bg-[var(--brand-soft)] border border-[var(--brand)]/20 rounded-xl p-3 text-xs text-[var(--brand)]">
                  ...
                  <textarea value={parseText} onChange={e => setParseText(e.target.value)}
                    ...
                    className="w-full bg-gray-50 border border-[var(--line)] focus:border-[var(--brand)] rounded-xl p-3 outline-none text-xs font-mono text-gray-700 resize-y transition-all disabled:opacity-60" />
                  ...
                  <div className="flex justify-between text-xs text-gray-400">
                    <span>{parseText.length} / 12,000</span><span className="font-medium text-[var(--brand)]">3 credits</span>
                  </div>
                </>
              ) : (
                <>
                          ...
                          <div key={i} className="bg-gray-50 rounded-xl border border-gray-100 overflow-hidden">
                            {editingIdx === i ? (
                              ...
                              <div className="p-3 space-y-2">
                                <div>
                                  <label className="text-[10px] font-semibold text-gray-500 uppercase tracking-widest">Câu hỏi</label>
                                  <textarea value={editDraft.question} onChange={e => setEditDraft({ ...editDraft, question: e.target.value })}
                                    rows={2} className="w-full text-xs border border-[var(--brand)]/40 rounded-lg p-2 font-medium outline-none resize-none mt-1 bg-white focus:border-[var(--brand)]" />
                                </div>
                                {editDraft.options && editDraft.options.length > 0 && (
                                  <div>
                                    <label className="text-[10px] font-semibold text-gray-500 uppercase tracking-widest">Đáp án (chọn ✓ làm đáp án đúng)</label>
                                    <div className="space-y-1 mt-1">
                                      {editDraft.options.map((opt: string, j: number) => (
                                        <div key={j} className="flex items-center gap-2">
                                          <button onClick={() => setEditDraft({ ...editDraft, answer: opt })}
                                            className={`w-5 h-5 rounded border-2 flex-shrink-0 flex items-center justify-center transition-all ${editDraft.answer === opt ? "bg-[var(--brand)] border-[var(--brand)] text-white" : "border-gray-300"}`}>
                                ...
                                {(!editDraft.options || editDraft.options.length === 0) && (
                                  <div>
                                    <label className="text-[10px] font-semibold text-gray-500 uppercase tracking-widest">Đáp án đúng</label>
                                    <input value={editDraft.answer} onChange={e => setEditDraft({ ...editDraft, answer: e.target.value })}
                                      className="w-full text-xs border border-[var(--brand)]/40 rounded-lg p-2 outline-none mt-1 font-medium bg-white focus:border-[var(--brand)]" />
                                  </div>
                                )}
                                <div className="flex gap-2 pt-1">
                                  <button onClick={saveEdit} className="flex-1 py-1.5 bg-[var(--brand)] text-white rounded-lg font-semibold text-xs flex items-center justify-center gap-1">
                                    <Check size={12} /> Lưu
                                  </button>
                                  <button onClick={cancelEdit} className="flex-1 py-1.5 bg-gray-100 text-gray-600 rounded-lg font-semibold text-xs">Hủy</button>
                                </div>
                              </div>
                            ) : (
                              /* Display mode */
              ...
              {parseTab === "ai" ? (
                <>
                  ...
                  <button onClick={handleAIParse} disabled={parsing || !parseText.trim()}
                    className="flex-1 sm:flex-none sm:px-8 py-2.5 rounded-xl font-semibold text-sm text-white bg-[var(--brand)] hover:bg-[var(--brand-dark)] transition flex items-center justify-center gap-1.5 shadow-lg disabled:opacity-40">
                ...
                <button onClick={practiceLocalQuestions}
                  className="flex-1 py-2.5 rounded-xl font-semibold text-sm text-white bg-[var(--brand)] hover:bg-[var(--brand-dark)] transition flex items-center justify-center gap-1.5 shadow-lg">
