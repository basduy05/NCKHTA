    ...
    <div className="space-y-6">
      ...
      {reviewExam ? (
        <div className="space-y-6 animate-in fade-in duration-300">
          <div className="bg-[var(--brand)] rounded-[var(--r-xl)] p-6 text-white flex items-center justify-between shadow-lg">
            <div className="flex items-center gap-5">
              <div className="bg-white/20 p-3 rounded-xl"><Eye size={28} /></div>
              <div>
                <h3 className="font-bold text-xl tracking-tight">Xem lại bài thi</h3>
            ...
            {reviewExam.exam_data?.questions?.map((q: any, idx: number) => {
              ...
              return (
                <div key={idx} className={`rounded-[2rem] p-8 border-2 transition-all ${isCorrect ? 'border-green-100 bg-green-50/20' : 'border-red-100 bg-red-50/20'}`}>
                  <p className="font-semibold text-lg mb-6 text-slate-800 flex items-start gap-4">
                    ...
                    {options.map((opt: any, oIdx: number) => {
                      ...
                      return (
                        <div key={oIdx} className={`p-4 rounded-2xl border-2 text-sm font-bold flex items-center gap-3 transition-all ${isOptCorrect ? 'border-green-500 bg-white text-green-700 shadow-md shadow-green-100' : isUserChoice ? 'border-red-500 bg-white text-red-700 shadow-md shadow-red-100' : 'border-gray-50 bg-gray-50/50 text-slate-500'}`}>
                          <span className={`w-7 h-7 rounded-xl flex items-center justify-center text-xs font-semibold ${isOptCorrect ? 'bg-green-500 text-white' : isUserChoice ? 'bg-red-500 text-white' : 'bg-white border border-gray-200 text-slate-400'}`}>{String.fromCharCode(65 + oIdx)}</span>
                          {renderValue(opt)}
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-[var(--brand)] rounded-[var(--r-lg)] flex items-center justify-center shadow-sm">
                    <Award className="text-white" size={24} />
                  </div>
                  <div>
                    <h2 className="text-2xl font-bold tracking-tight text-slate-900">Trung tâm luyện thi</h2>
                    <p className="text-slate-400 font-medium uppercase tracking-widest text-xs mt-1">Nâng cấp kỹ năng • Chinh phục mục tiêu</p>
                  </div>
                </div>
                <button onClick={() => { setShowHistory(!showHistory); setPractice(null); }} className={`flex items-center gap-2 px-5 py-3 rounded-2xl font-semibold text-sm transition-all border-2 ${showHistory ? 'bg-[var(--brand)] text-white border-[var(--brand)] shadow-lg shadow-blue-200' : 'bg-white text-slate-600 border-slate-100 hover:border-[var(--brand)]/20'}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                <div className="md:col-span-4 grid grid-cols-2 gap-2">
                    ...
                    <div className="relative">
                      <p className="absolute -top-2.5 left-4 px-2 bg-white text-[10px] font-semibold text-blue-400 uppercase tracking-widest z-20">Nguồn đề</p>
                      <select value={source} onChange={e => setSource(e.target.value as any)} className="w-full h-14 bg-slate-50 border-2 border-slate-100 rounded-2xl px-5 outline-none text-slate-900 font-semibold text-sm appearance-none focus:border-blue-300 transition-all">
                  ...
                  {testType === "TOEIC" ? (
                    ...
                    <div className="flex gap-2">
                      {[
                        ...
                        <button key={s.val} onClick={() => setSkill(s.val)} className={`flex-1 h-14 flex flex-col items-center justify-center rounded-2xl border-2 transition-all ${skill === s.val ? 'bg-[var(--brand)] text-white border-[var(--brand)] shadow-lg shadow-blue-100' : 'bg-slate-50 text-slate-400 border-slate-100 hover:bg-white hover:border-[var(--brand)]/30'}`}>
                          {s.icon} <span className="text-[10px] font-semibold uppercase mt-1 leading-none">{s.label}</span>
                        </button>
                      ))}
                    </div>
                  )}
                ...
                <div className="md:col-span-2">
                   <button onClick={generatePractice} disabled={loading} className="w-full h-14 bg-[var(--brand)] hover:bg-[var(--brand-dark)] text-white rounded-2xl disabled:opacity-50 font-semibold shadow-md transition-all text-sm tracking-widest active:scale-95">
      ...
      {showHistory && (
        <div className="app-card overflow-hidden animate-in slide-in-from-top-4 duration-300">
          <div className="p-8 border-b border-gray-50 bg-gray-50/50 flex items-center justify-between">
            <h3 className="font-bold text-lg text-slate-900 flex items-center gap-3">
              <div className="w-10 h-10 bg-[var(--brand-soft)] text-[var(--brand)] rounded-xl flex items-center justify-center"><History size={20} /></div>
            ...
            <div className="divide-y divide-slate-50">
              {examHistory.map((exam: any) => (
                <div key={exam.id} className="p-6 hover:bg-slate-50/50 transition flex items-center justify-between group cursor-pointer" onClick={() => loadExamReview(exam.id)}>
                  <div className="flex items-center gap-6">
                    <div className={`w-14 h-14 rounded-2xl flex flex-col items-center justify-center font-semibold text-sm shadow-sm transition-transform group-hover:scale-110 ${exam.score >= exam.max_score * 0.8 ? 'bg-green-100 text-green-700' : exam.score >= exam.max_score * 0.5 ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'}`}>
                    ...
                    <div>
                      <p className="font-semibold text-slate-900 text-lg leading-tight mb-1">{renderValue(exam.title || `${exam.test_type} ${exam.skill || ''}`)}</p>
                      <div className="flex items-center gap-3">
                          <span className="px-2 py-0.5 bg-slate-100 rounded text-[10px] font-semibold text-slate-500 uppercase tracking-widest">{exam.test_type}</span>
                  ...
                  <div className="flex items-center gap-6">
                    <div className="text-right">
                       <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Questions</p>
                       <p className="text-sm font-semibold text-slate-700">{exam.score}/{exam.max_score}</p>
                    </div>
                    <div className="w-10 h-10 bg-slate-50 text-slate-300 rounded-xl flex items-center justify-center group-hover:bg-[var(--brand)] group-hover:text-white transition-all">
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center py-16 space-y-6">
          <div className="relative">
             <div className="animate-spin rounded-full h-16 w-16 border-4 border-[var(--brand-soft)] border-t-[var(--brand)]"></div>
             <Sparkles className="absolute inset-0 m-auto text-[var(--brand)] animate-pulse" size={24} />
          </div>
          <div className="text-center">
            <p className="text-slate-900 font-bold text-lg mb-1 tracking-tight">AI đang chuẩn bị đề thi...</p>
            <p className="text-slate-400 font-medium uppercase tracking-widest text-[10px]">Vui lòng đợi trong giây lát</p>
          </div>
        </div>
      )}
      ...
      {practice && (skill === "reading" || skill === "listening" || (testType === "TOEIC" && toeicPart)) && !loading && !submitted && (
        <div className="app-card overflow-hidden animate-in fade-in slide-in-from-bottom-6 duration-500">
          <div className="p-6 sm:p-8 border-b border-[var(--line)]">
             <div className="flex items-center gap-4 mb-3">
                <div className="px-3 py-1 bg-[var(--brand)] text-white rounded-full text-[10px] font-semibold uppercase tracking-widest leading-none">{testType} {toeicPart || skill}</div>
                <div className="text-slate-400 font-medium text-xs flex items-center gap-1"><Clock size={14} /> {practice.questions?.length || 0} Questions</div>
             </div>
             <h3 className="text-2xl font-bold text-slate-900 tracking-tight leading-tight">{renderValue(practice.title || "Luyện tập tổng hợp")}</h3>
          </div>
          ...
          <div className="p-10">
            ...
            <div className="space-y-8">
              {practice.questions?.map((q: any, idx: number) => {
                const options = getOptionsArray(q);
                return (
                  <div key={idx} className="bg-white border border-[var(--line)] rounded-[var(--r-xl)] p-6 hover:border-[var(--brand)]/30 transition-all group">
                    <p className="font-bold text-base mb-5 text-slate-800 flex items-start gap-4">
                      <span className="w-8 h-8 rounded-xl bg-[var(--surface-3)] text-slate-400 flex items-center justify-center text-sm shrink-0 group-hover:bg-[var(--brand)] group-hover:text-white transition-all">{idx + 1}</span>
                      {renderValue(q.question || q.q)}
                    </p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pl-14">
                      {options.map((opt: any, oIdx: number) => (
                        <label key={oIdx} className={`relative flex items-center gap-3 p-4 rounded-[var(--r-lg)] border-2 cursor-pointer transition-all ${answers[idx] === oIdx ? 'border-[var(--brand)] bg-[var(--brand-soft)]/30' : 'border-[var(--surface-3)] bg-[var(--surface-2)] hover:border-[var(--brand)]/30 hover:bg-white'}`}>
                          <input
                            type="radio"
                            name={`q-${idx}`}
                            ...
                            className="hidden"
                          />
                          <div className={`w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-semibold transition-all ${answers[idx] === oIdx ? 'bg-[var(--brand)] text-white' : 'bg-white border border-[var(--line)] text-slate-400'}`}>
                            {String.fromCharCode(65 + oIdx)}
                          </div>
                          <span className={`text-sm font-medium ${answers[idx] === oIdx ? 'text-[var(--ink-1)]' : 'text-slate-500'}`}>{renderValue(opt)}</span>
                          {answers[idx] === oIdx && <div className="ml-auto w-2 h-2 rounded-full bg-[var(--brand)]"></div>}
                        </label>
                      ))}
                    </div>
                  </div>
              ...
              {!submitted && (
                <div className="pt-8 flex justify-center">
                  <button onClick={submitAnswers} className="bg-[var(--brand)] hover:bg-[var(--brand-dark)] text-white py-4 px-12 rounded-[var(--r-xl)] font-semibold text-base shadow-md transition-all active:scale-95">NỘP BÀI ✨</button>
                </div>
              )}
            </div>
          </div>
      ...
      {practice && skill === "speaking" && !loading && !submitted && (
        <div className="app-card p-8 sm:p-10 text-center animate-in zoom-in-95 duration-500">
          <div className="relative z-10 max-w-4xl mx-auto">
            <div className="inline-flex p-5 bg-purple-50 text-purple-600 rounded-[var(--r-xl)] mb-6 border border-purple-100">
                <Mic size={40} className={speakingStage === "speaking" ? "animate-pulse" : ""} />
            </div>
            <h3 className="font-bold text-2xl mb-3 text-slate-900 tracking-tight">{renderValue(practice.topic || practice.passage || practice.title)}</h3>
            ...
            {speakingStage === "idle" && (
                <button
                  ...
                  className="bg-[var(--brand)] hover:bg-[var(--brand-dark)] text-white px-10 py-4 rounded-[var(--r-xl)] font-semibold text-base shadow-md hover:scale-105 active:scale-95 transition-all"
                >
            ...
            {(speakingStage === "prep" || speakingStage === "speaking") && (
                <div className="space-y-10 animate-in zoom-in-95 duration-300">
                    <div className="flex flex-col items-center">
                        <div className={`text-6xl font-bold tabular-nums mb-4 ${timer < 10 ? 'text-red-500 animate-pulse' : 'text-[var(--brand)]'}`}>
                            {Math.floor(timer / 60)}:{String(timer % 60).padStart(2, '0')}
                        </div>
                        <div className="px-6 py-2 bg-slate-100 rounded-full text-xs font-semibold uppercase tracking-widest text-slate-400">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
                        <div className="bg-[var(--surface-2)] p-6 rounded-[var(--r-xl)] border border-[var(--line)]">
                            <h4 className="font-semibold text-slate-900 mb-6 flex items-center gap-3 text-xs uppercase tracking-widest">
            ...
            {speakingStage === "result" && (
                <div className="space-y-10 animate-in fade-in duration-500 text-left">
                    <div className="bg-green-50 p-8 rounded-[var(--r-xl)] border border-green-100 relative overflow-hidden text-left">
                        <h4 className="font-semibold text-green-800 mb-6 flex items-center gap-3 text-xs uppercase tracking-widest">
                            <CheckCircle2 size={24} /> Bài mẫu tham khảo:
                        </h4>
                        <p className="text-green-900 leading-relaxed italic text-base font-medium relative z-10">&ldquo;{renderValue(practice.model_answer)}&rdquo;</p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="app-card p-6 text-left">
                            <h4 className="font-semibold text-amber-600 mb-5 flex items-center gap-2 text-xs uppercase tracking-widest">Lời khuyên từ AI</h4>
                            <ul className="space-y-4">
                        ...
                        <div className="app-card p-6 text-left">
                             <h4 className="font-semibold text-[var(--brand)] mb-5 flex items-center gap-2 text-xs uppercase tracking-widest">Tiêu chí đánh giá</h4>
      ...
      {practice && skill === "writing" && !loading && !submitted && (
        <div className="app-card p-6 sm:p-10 max-w-5xl mx-auto animate-in slide-in-from-bottom-8 duration-500">
          <div className="flex items-center gap-4 mb-8">
            <div className="p-3 bg-[var(--brand-soft)] text-[var(--brand)] rounded-[var(--r-lg)]"><PenTool size={24} /></div>
            <div>
              <h3 className="text-xl font-bold text-slate-900 tracking-tight">{renderValue(practice.title || "Luyện viết cùng AI")}</h3>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between gap-3 mb-2">
                      <span className={`text-[10px] font-semibold uppercase tracking-widest ${isActive ? "text-blue-600" : "text-slate-400"}`}>
                    ...
                    <p className={`font-semibold text-base leading-tight ${isActive ? "text-slate-900" : "text-slate-700"}`}>
          ...
          <div className="bg-slate-50 p-8 rounded-[2rem] border border-slate-100 mb-10">
            <h4 className="font-semibold text-slate-400 mb-4 uppercase text-[10px] tracking-widest flex items-center gap-2">
            ...
            {currentWritingTask && (
              <div className="mt-6 flex flex-wrap gap-2">
                <span className="px-3 py-2 bg-white rounded-xl border border-slate-200 text-[11px] font-semibold text-slate-500">
          ...
          <div className="relative">
            ...
            />
            <div className="absolute bottom-6 right-8 bg-slate-900 text-white px-4 py-2 rounded-xl text-[10px] font-semibold uppercase tracking-widest shadow-lg">
          ...
          <div className="mt-10 flex flex-col items-center gap-3">
            <button
              ...
              className="bg-[var(--brand)] hover:bg-[var(--brand-dark)] text-white py-4 px-12 rounded-[var(--r-xl)] font-semibold text-base shadow-md transition-all active:scale-95 disabled:opacity-30"
            >
      ...
      {submitted && !loading && (
        <div className="max-w-5xl mx-auto animate-in zoom-in-95 duration-500">
          {skill === "writing" && evalResult ? (
            <div className="space-y-8">
              <div className="bg-slate-900 rounded-[var(--r-2xl)] p-8 sm:p-10 text-white shadow-xl relative overflow-hidden">
                <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
                  <div className="flex-1">
                    <div className="inline-flex items-center gap-2 bg-blue-500/20 text-blue-300 px-4 py-1.5 rounded-full border border-blue-500/30 text-[10px] font-semibold uppercase tracking-widest mb-4">Writing Analysis</div>
                    <h3 className="text-2xl font-bold mb-3">Kết Quả Đánh Giá</h3>
                  ...
                  <div className="shrink-0 flex flex-col items-center">
                    <div className="w-44 h-44 rounded-full border-[10px] border-blue-500/30 flex flex-col items-center justify-center bg-white/5 backdrop-blur-xl">
                      <span className="text-[10px] font-semibold text-blue-300 uppercase">Overall</span>
                      <span className="text-5xl font-bold tabular-nums">{renderValue(evalResult.overall_band)}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="app-card p-8">
                  <h4 className="font-semibold text-slate-900 mb-6 uppercase text-xs tracking-widest flex items-center gap-3">
                    <BarChart3 className="text-[var(--brand)]" /> Tiêu chí chi tiết
                  </h4>
                  <div className="space-y-5">
                    {Object.entries(evalResult.criteria || {}).map(([key, value]: [string, any]) => (
                      <div key={key}>
                        <div className="flex justify-between items-center mb-1.5">
                          <span className="font-medium text-slate-600 capitalize text-xs tracking-tight">{key.replace('_', ' ')}</span>
                          <span className="font-semibold text-[var(--brand)] text-xs">{renderValue(value.score)}/9</span>
                        </div>
                        <div className="w-full h-1.5 bg-[var(--surface-3)] rounded-full overflow-hidden">
                          <div className="h-full bg-[var(--brand)] rounded-full" style={{ width: `${(value.score / 9) * 100}%` }}></div>
                        </div>
                        <p className="text-xs text-slate-400 mt-1.5 font-medium leading-relaxed">{renderValue(value.feedback_vn)}</p>
                      </div>
                    ))}
                  </div>
                </div>
                <div className="space-y-5">
                   <div className="app-card p-8">
                      <h4 className="font-semibold text-green-600 mb-5 uppercase text-xs tracking-widest flex items-center gap-3"><CheckCircle2 size={14} /> Điểm mạnh</h4>
                   ...
                   <div className="bg-[var(--brand)] p-8 rounded-[var(--r-xl)] text-white">
                      <h4 className="font-semibold mb-5 uppercase text-xs tracking-widest flex items-center gap-3"><Sparkles size={14} /> Cần cải thiện</h4>
                      <ul className="space-y-3">
                        {evalResult.improvements?.map((im: string, i: number) => (
                           <li key={i} className="text-sm font-medium opacity-90 border-l-2 border-white/30 pl-3">{renderValue(im)}</li>
                        ))}
                      </ul>
                   </div>
                </div>
              ...
              <div className="pt-8 text-center">
                 <Button onClick={() => { setPractice(null); setSubmitted(false); setEvalResult(null); }} intent="brand" size="lg">LUYỆN TẬP BÀI KHÁC</Button>
              </div>
            </div>
          ) : (
            <div className="app-card p-10 sm:p-12 text-center">
              <div className="w-20 h-20 bg-[var(--brand-soft)] rounded-[var(--r-xl)] flex items-center justify-center mx-auto mb-6">
                <Trophy size={48} className="text-yellow-500 animate-bounce" />
              </div>
              <h3 className="text-2xl font-bold mb-2 text-[var(--ink-1)] tracking-tight">Hoàn Thành!</h3>
              <div className="text-4xl font-bold tabular-nums text-[var(--brand)] my-4">{score}<span className="text-xl text-[var(--ink-3)]">/{practice?.questions?.length || 0}</span></div>
              <p className="text-sm font-medium text-[var(--ink-2)] mb-8 max-w-sm mx-auto">Chúc mừng bạn đã hoàn thành bài luyện tập. Hãy tiếp tục để đạt kết quả cao hơn!</p>
              <div className="flex flex-col sm:flex-row justify-center gap-3">
                <Button onClick={() => { setPractice(null); setSubmitted(false); setAnswers({}); }} intent="brand" size="lg">LUYỆN TẬP TIẾP</Button>
      ...
      {!practice && !loading && !showHistory && (
        <div className="app-card p-12 sm:p-16 text-center">
          <div className="w-16 h-16 bg-[var(--surface-3)] rounded-[var(--r-xl)] flex items-center justify-center mx-auto mb-6 text-slate-300">
             <Layers size={32} />
          </div>
          <h3 className="text-xl font-bold text-slate-800 mb-3 tracking-tight">Sẵn sàng để luyện tập?</h3>
          <p className="text-slate-400 font-medium max-w-sm mx-auto mb-8">Chọn chứng chỉ và kỹ năng bạn muốn cải thiện, sau đó nhấn nút "Bắt đầu" để AI chuẩn bị đề thi cho bạn.</p>
          <div className="inline-flex items-center gap-4 p-2 bg-slate-50 rounded-2xl border border-slate-100">
             ...
             <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest mr-2">Hệ thống AI đã sẵn sàng</span>
          </div>
        </div>
      )}
      </>
